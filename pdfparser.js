 ...
 * @info Private methods accessible using the [funcName].call(this, ...) syntax
   ...
   */
  #startParsingPDF(buffer, pageNum) {
    this.#data = {};
    ...
    this.#PDFJS.parsePDFData(buffer || PDFParser.#binBuffer[this.binBufferKey], this.#password, pageNum);
  }
  ...
  /**
     ...
     * @returns {boolean}
     */
  #processBinaryCache(pageNum) {
    if (this.binBufferKey in PDFParser.#binBuffer) {
      this.#startParsingPDF(undefined, pageNum);
      return true;
    }
  ...
  /**
     ...
     */
  async loadPDF(pdfFilePath, verbosity, pageNum) {
    ...
    try {
      ...
      if (this.#processBinaryCache(pageNum))
        return;
