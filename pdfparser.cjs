...
class PDFJSClass extends events.EventEmitter {
  ...
  parsePDFData(arrayBuffer, password, page) {
    this.pdfDocument = null;

    const parameters = { password: password, data: arrayBuffer };
    PDFJS.getDocument(parameters).then(
      (pdfDocument) => this.load(pdfDocument, 1, page),
  ...
  load(pdfDocument, scale, page) {
    this.pdfDocument = pdfDocument;

    return this.loadMetaData().then(
      () => {
        const pages = this.loadPages(page);
        return pages;
      },
...
class PDFParser extends events.EventEmitter { // inherit from event emitter
  ...
  #startParsingPDF(buffer, page) {
    ...
    this.#PDFJS.parsePDFData(buffer || PDFParser.#binBuffer[this.binBufferKey], this.#password, page);
  }

  #processBinaryCache(page) {
    if (this.binBufferKey in PDFParser.#binBuffer) {
      this.#startParsingPDF(undefined, page);
      return true;
    }
  ...
  async loadPDF(pdfFilePath, verbosity, page) {
    ...
    try {
      ...
      if (this.#processBinaryCache(page))
        return;
