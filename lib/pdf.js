...
export default class PDFJSClass extends EventEmitter {
  ...
  parsePDFData(arrayBuffer, password, pageNum) {
    this.pdfDocument = null;

    const parameters = { password: password, data: arrayBuffer };
    PDFJS.getDocument(parameters).then(
      (pdfDocument) => this.load(pdfDocument, 1, pageNum),
  ...
  load(pdfDocument, scale, pageNum) {
    this.pdfDocument = pdfDocument;

    return this.loadMetaData().then(
      () => this.loadPages(pageNum),
  ...
  loadPages(pageNum) {
    ...
    for (let i = 1; i <= pagesCount; i++) {
      if (pageNum === undefined || ((pageNum + 1) === i)) {
    ...
    return pagesPromise.then(
      (promisedPages) => this.parsePage(promisedPages, 0, 1.5, pageNum !== undefined),
      (error) => this.raiseErrorEvent('pagesPromise error: ' + error)
    );
  }

  parsePage(promisedPages, id, scale, onePageOnly = false) {
    ...
    function continueOnNextPage() {
      ...
      if (onePageOnly === true || id === this.pdfDocument.numPages - 1) {
    ...
    pageParser.parsePage(
      () => {
        // console.log('pageParser.parsePage', pdfPage.annotationsPromise);
        // console.log('pageParser.parsePage2', pageParser);

        pdfPage.annotationsPromise.then((annotations) => {
          ...
          // console.log('pageParser.parsePage');
          // this.needRawText = true;
          // pdfPage.getTextContent().then(
          //   (textContent) => {
          //     console.log({ textContent });
          //   });
          // const raw = this.getRawTextContent();
          // console.log({ raw });
          if (this.needRawText) {
            pdfPage.getTextContent().then(
              (textContent) => {
